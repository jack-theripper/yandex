...
## Геттеры, методы get и getContents

```php
public mixed Класс::get(string $key [, mixed $default = null])
```
> **Описание:**
> **key** - индекс, например 'total_space'
> **default** - значение по умолчанию, если **key** не существует. Может принимать **Closure** - функция будет выполнена с параметром текущего контекста (например *Disk*, *Resource* и т.д.) и возвращен результат. 
```php
...
/* array */
```
Этот метод доступен везде и это описание справедливо для всего SDK.

```php
public mixed Класс::getContents( void )
```
Получает всю информацию в виде массива, метод доступен везде.
...
```php
public mixed Класс::has([string $key])
```

Метод доступен везде, но с разными параметрами. Проверяет существует ли свойство, а также в контексте ресурса (например Disk\Resource) может проверить существует ли ресурс на диске. Возвращает **boolean**.
...
Ресурсы - файлы, папки, которые расположены на диске (не в корзине). 
```php
public Disk\Resource Disk::resource([string $path = null [, int $limit = 20 [, int $offset = 0]]])
```
> **Описание:**

> **path** - путь к новому либо уже существующему ресурсу, NULL Список всех файлов. Параметр подразумевает текущее или будущее расположение ресурса.
> **limit** - количество файлов или вложенных ресурсов (если это папка) на выборку.
> **offset** - смещение.
> Возвращает объект Disk\Resource или массив.

#### <i class="icon-file"></i> Получить список всех файлов, исключая вложенность в папки

Вызов 'resource' без параметров тоже самое, что и с параметром **path** = *NULL* и возвращает массив.
```php
$files = $disk->resource();
```

 С параметрами **limit** = 10 и **offset** = 5
 
```php
$files = $disk->resource(null, 10, 5);
```
Массив объектов *Disk\Resource*. Поэтому в цикле доступны *delete*, *upload* и прочие методы.
```php
foreach ($files as $file)
{
	// getContents в отличии от get получает всю информацию в виде массива
	var_dump($file->getContents());	
}
/* array (size=11)
...
Так же осуществляется с помощью метода Disk::resource, который описан выше.
```php
